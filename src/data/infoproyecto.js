export const infoproyecto = [
    {
        nombre: "PIAC",
        img: "/img/icon_PIAC.webp",
        objetivo: "Actualizar y mejorar La pagina web del PIAC (Patrimonio de la Industria Alemana para la Cultura, A.C.) para facilitar el acceso a información sobre sus servicios y actividades.",
        herramientas: [
            { herramienta: "Visual Studio Code" },
            { herramienta: "Astro Build" },
            { herramienta: "Tailwind CSS" },
        ],
        resultados: "Pagina web actualizada ",
        page: "https://www.piac.org.mx",
        target: "_target"

    },
    {
        nombre: "AppMedik",
        img: "/img/appmedik.png",
        objetivo: "Desarrollar una aplicación móvil pediátrica para la gestión de citas médicas y la dosificación de medicamentos.",
        herramientas: [
            { herramienta: "Android Studio 2023.1.13" },
            { herramienta: "Node.js v.18.16.0" },
            { herramienta: "MongoDB Atlas v.4.4.6" },
            { herramienta: "AWS EC2 v.2.0.1" }
        ],
        resultados: "Aplicación Beta",
        page: "/appmedik",
        target: "_self"
    },
    {
        nombre: "API CRUD para Restaurante “El Zarape”",
        objetivo: "Crear servicios RESTful para operaciones CRUD en el sistema de punto de venta del restaurante.",
        img: "/img/El-Zarape.png",
        herramientas: [
            { herramienta: "MySQL 8.0.38" },
            { herramienta: "Apache NetBeans IDE 24" },
            { herramienta: "Apache TomCat 10.1.13" },
            { herramienta: "Postman v.11.31.4" },
            { herramienta: "IntelliJIDEA 2024.2.5" }
        ],
        resultados: "API REST para gestionar datos en una base de datos.",

    },
    {
        nombre: "Sistema DAQ para Monitoreo de Sensores",
        objetivo: "Implementar un sistema que interactúe con sensores de temperatura y humedad, almacenando datos en una base de datos mediante una interfaz gráfica.",
        img: "/img/DAQ.png",
        herramientas: [
            { herramienta: "Arduino Uno" },
            { herramienta: "Arduino IDE 2.3.4" },
            { herramienta: "Apache NetBeans IDE 24" },
            { herramienta: "Dweet.io" }
        ],
        resultados: "Monitoreo en tiempo real con recolección de datos.",
        page: "/detalle-proyecto",
        target: "_self"

    },
    {
        nombre: "Formulario de Usuarios en Android (CRUD)",
        objetivo: "Implementar un sistema CRUD para gestionar usuarios en dispositivos móviles.",
        img: "/img/CRUD.png",
        herramientas: [
            { nombre: "IDE", herramienta: "Android Studio 2023.1.13" },
            { nombre: "Base de Datos", herramienta: "SQLite 2.4.0" }
        ],
        resultados: "Aplicación móvil funcional con base de datos integrada.",
        page: "/detalle-proyecto",
        target: "_self"

    }, {
        nombre: "Formulario de Usuarios en Android (CRUD)",
        objetivo: "Implementar un sistema CRUD para gestionar usuarios en dispositivos móviles.",
        img: "/img/CRUD-movil.png",
        herramientas: [
            { nombre: "IDE", herramienta: "Android Studio 2023.1.13" },
            { nombre: "Base de Datos", herramienta: "SQLite 2.4.0" }
        ],
        resultados: "Aplicación móvil funcional con base de datos integrada.",
        page: "/detalle-proyecto",
        target: "_self"

    }

];